...
runners.forEach(element => {
    ticketpriceArray.push(element.donation)
})

 ticketPriceTotal = ticketpriceArray.reduce((a,b) => {
    return a + b;
})
...
// Problem 1
// Get list of runners whose shirt size is small
let smallShirts = [];
runners.filter(element => {
    if (element.shirt_size === "S"){
        smallShirts.push(element)
    }
})
console.log(smallShirts);

// Problem 2
//Get list of runners email
let email = [];
runners.forEach(element => {
...
// Problem 3
//Get list of runners whose donation is above 200
let donationSize = [];
runners.filter(element => {
    if (element.donation > 200){
        donationSize.push(element)
    }
})
